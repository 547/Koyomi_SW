...
public enum SelectionMode {
    ...
    public enum Style { case background, circle, line, oval }
}
...
private extension Koyomi {
    ...
    func configure(_ cell: KoyomiCell, at indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            let type = DateModel.WeekType(indexPath)
            textColor = {
            ...
            style = {
                ...
                var ovalLineSequencePosition: KoyomiCell.CellStyle.OvalLineSequencePosition {
                    var result = KoyomiCell.CellStyle.OvalLineSequencePosition.none
                    guard isSelected else {
                        return result
                    }
                    let leftIndexPath = IndexPath(item: indexPath.item - 1, section: indexPath.section)
                    let rightIndexPath = IndexPath(item: indexPath.item + 1, section: indexPath.section)
                    if leftIndexPath.item >= 0, rightIndexPath.item < numberOfItems(inSection: indexPath.section), model.isSelect(with: leftIndexPath), model.isSelect(with: rightIndexPath) {
                        result = .middle
                        if let weakType = type {
                            if weakType == .sunday {
                                result = .left
                            }else if weakType == .saturday {
                                result = .right
                            }
                        }
                    }else if leftIndexPath.item < 0 || (leftIndexPath.item >= 0 && !model.isSelect(with: leftIndexPath)), rightIndexPath.item < numberOfItems(inSection: indexPath.section), model.isSelect(with: rightIndexPath) {
                        result = .left
                        if let weakType = type {
                            if weakType == .saturday {
                                result = .none
                            }
                        }
                    }else if rightIndexPath.item >= numberOfItems(inSection: indexPath.section) || (rightIndexPath.item < numberOfItems(inSection: indexPath.section) && !model.isSelect(with: rightIndexPath)), leftIndexPath.item >= 0, model.isSelect(with: leftIndexPath) {
                        result = .right
                        if let weakType = type {
                            if weakType == .sunday {
                                result = .none
                ...
                case (.multiple(style: .oval), true):
                    return .oval(position: ovalLineSequencePosition)
